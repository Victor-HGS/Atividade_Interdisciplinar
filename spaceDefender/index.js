...
const nave = new Audio('sons/naveexplodindo.mp3');
...
function animate() {
  ...
  // spawn bombs
  if (frames % 600 === 0 && bombs.length < 3) {
    bombs.push(
      new Bomb({
        position: {
  ...
  if((keys.ArrowLeft.pressed || keys.a.pressed) && player.position.x >= 0) {
        player.velocity.x = -7
        player.rotation = -0.15
    } else if((keys.ArrowRight.pressed || keys.d.pressed) && player.position.x + player.width <= canvas.width) {
        player.velocity.x = 7
        player.rotation = 0.15
    }else {
        player.velocity.x = 0
...
const tiro = new Audio('sons/tirolaser.mp3');
